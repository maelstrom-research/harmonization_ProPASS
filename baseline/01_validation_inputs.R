...
download.file(
  ...
  destfile = paste0("input_documents/data_processing_elements-",
                    checks$harmo_group, 
                    "-github.xlsx"),
  mode = "wb")
dpe_github <- read_excel_allsheets(paste0("input_documents/data_processing_elements-",
                                          checks$harmo_group, 
                                          "-github.xlsx"))
...
if(nrow(dpe_list) == 1){
  ...
  if(identical(dpe_local, dpe_github)){
    file.remove(paste0("input_documents/data_processing_elements-",
                       checks$harmo_group, 
                       "-github.xlsx"))
  }else{
    file.rename(
    ...
    file.rename(
      from = paste0("input_documents/data_processing_elements-",
                    checks$harmo_group, 
                    "-github.xlsx"),
      to = paste0("input_documents/data_processing_elements-",
                  checks$harmo_group, 
    ...
    if(checks$dataschema_uptodate != "Dataschema updated!"){
      ...
      if(specific == 2){
        ...
        write_excel_allsheets(dpe_github,
                              paste0("input_documents/data_processing_elements-",
                                     checks$harmo_group, 
                                     "-", format(checks$time_stamp, "%Y-%m-%d"), ".xlsx")
        )
        rm(var_changed)
