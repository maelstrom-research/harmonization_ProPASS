...
#---- Get the dataschema ----
download.file(
  url = "https://github.com/maelstrom-research/harmonization_ProPASS/raw/master/linkage/dataschema_ProPASS_linkage.xlsx",
  destfile = "input_documents/dataschema_ProPASS_linkage_github.xlsx",
  mode = "wb")

dataschema_github <- read_excel_allsheets("input_documents/dataschema_ProPASS_linkage_github.xlsx")

# Compare dataschema versions
if(file.exists("input_documents/dataschema_ProPASS_linkage.xlsx")){
  dataschema_local <- read_excel_allsheets("input_documents/dataschema_ProPASS_linkage.xlsx")
  ...
  if(test_all_equal){
    ...
    invisible(file.remove("input_documents/dataschema_ProPASS_linkage_github.xlsx"))
  }else{
    file.rename(from = "input_documents/dataschema_ProPASS_linkage.xlsx",
                to = paste0("archive/dataschema_ProPASS_linkage_archived-", format(checks$time_stamp,"%Y-%m-%H%M%S"), ".xlsx"))
    file.rename(from = "input_documents/dataschema_ProPASS_linkage_github.xlsx",
                to = "input_documents/dataschema_ProPASS_linkage.xlsx")
    message("
...
download.file(
  ...
  destfile = paste0("input_documents/data_processing_element_linkage-",
                    ...
                    "-github.xlsx"),
  mode = "wb")
dpe_github <- read_excel_allsheets(paste0("input_documents/data_processing_element_linkage-",
                                          checks$harmo_group, 
                                          "-github.xlsx"))
...
if(nrow(dpe_list) == 1){
  ...
  if(identical(dpe_local, dpe_github)){
    file.remove(paste0("input_documents/data_processing_element_linkage-",
                       checks$harmo_group, 
                       "-github.xlsx"))
  }else{
    file.rename(
    ...
    file.rename(
      from = paste0("input_documents/data_processing_element_linkage-",
                    checks$harmo_group, 
                    "-github.xlsx"),
      to = paste0("input_documents/data_processing_element_linkage-",
                  checks$harmo_group, 
    ...
    if(checks$dataschema_uptodate != "Dataschema updated!"){
      ...
      if(specific == 2){
        ...
        write_excel_allsheets(dpe_github,
                              paste0("input_documents/data_processing_element_linkage-",
                                     checks$harmo_group, 
                                     "-", format(checks$time_stamp, "%Y-%m-%d"), ".xlsx")
        )
        rm(var_changed)
