...
build_wave_map <- function(data, datasets) {
  waves <- sort(unique(data$adm_wave))
  
  ## adm_dataset_id already resolves everything - no map needed
  if ("adm_dataset_id" %in% names(data) &&
      all(datasets %in% unique(data$adm_dataset_id))) {
    message("adm_dataset_id resolves every input_dataset; WAVE_MAP not needed.")
    return(c())
  }
  
  ## single dataset, single wave - unambiguous
  if (length(datasets) == 1 && length(waves) == 1) {
    message("Single dataset, single wave: ", datasets, " -> wave ", waves)
    return(setNames(waves, datasets))
  }
  
  ## equal counts: order datasets by the last number in their name
  num <- suppressWarnings(as.numeric(sub(".*?([0-9]+)[^0-9]*$", "\\1", datasets)))
  if (length(datasets) == length(waves) && !any(is.na(num)) && !any(duplicated(num))) {
    m <- setNames(waves[rank(num)], datasets)
    message("Inferred from numbers in names - VERIFY:")
    print(data.frame(input_dataset = datasets, number = num, adm_wave = as.numeric(m)))
    return(m)
  }
  
  stop("Cannot infer a mapping. DPE datasets: ", paste(datasets, collapse = ", "),
       ". Waves in data: ", paste(waves, collapse = ", "),
       ". Set WAVE_MAP by hand.", call. = FALSE)
}

verify_wave_map <- function(data, dpe, map) {
  waves <- sort(unique(data$adm_wave))
  score <- function(d, w) {
    vars <- dpe$dataschema_variable[dpe$input_dataset == d &
                                      dpe[["Mlstr_harmo::status"]] == "complete"]
    vars <- intersect(vars, names(data))
    if (!length(vars)) return(NA_real_)
    sub <- data[data$adm_wave == w, vars, drop = FALSE]
    mean(vapply(sub, function(col) all(is.na(col)), logical(1)))
  }
  tab <- outer(names(map), waves, Vectorize(score))
  dimnames(tab) <- list(names(map), paste0("wave_", waves))
  message("Fraction of 'complete' variables that are entirely NA (lower = better fit):")
  print(round(tab, 3))
  best <- waves[apply(tab, 1, which.min)]
  bad  <- names(map)[best != as.numeric(map)]
  if (length(bad))
    warning("Mapping disagrees with the data for: ", paste(bad, collapse = ", "),
            call. = FALSE)
  invisible(tab)
}
