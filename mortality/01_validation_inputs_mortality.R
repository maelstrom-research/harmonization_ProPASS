...
Make sure you have closed your Excel files.
...
#---- Get the dataschema ----
download.file(
  url = "https://github.com/maelstrom-research/harmonization_ProPASS/raw/master/mortality/dataschema_ProPASS_mortality.xlsx",
  destfile = "input_documents/dataschema_ProPASS_mortality_github.xlsx",
  mode = "wb")

dataschema_github <- read_excel_allsheets("input_documents/dataschema_ProPASS_mortality_github.xlsx")

# Compare dataschema versions
if(file.exists("input_documents/dataschema_ProPASS.xlsx")){
  dataschema_local <- read_excel_allsheets("input_documents/dataschema_ProPASS_mortality.xlsx")
  ...
  if(test_all_equal){
    ...
    invisible(file.remove("input_documents/dataschema_ProPASS_mortality_github.xlsx"))
  }else{
    file.rename(from = "input_documents/dataschema_ProPASS_mortality.xlsx",
                to = paste0("archive/dataschema_ProPASS_mortality_archived-", format(checks$time_stamp,"%Y-%m-%H%M%S"), ".xlsx"))
    file.rename(from = "input_documents/dataschema_ProPASS_mortality_github.xlsx",
                to = "input_documents/dataschema_ProPASS_mortality.xlsx")
    message("
...
# If the first time using the script
if(!file.exists("input_documents/dataschema_ProPASS_mortality.xlsx")){
  file.rename(from = "input_documents/dataschema_ProPASS_mortality_github.xlsx",
              to = "input_documents/dataschema_ProPASS_mortality.xlsx")
...
dpe_list <- file.info(list.files("input_documents",
                                 pattern = "data_processing_elements",
                                 full.names = TRUE
                                 )
                      ) %>%
...
download.file(
  url = paste0(
    "https://github.com/maelstrom-research/harmonization_ProPASS/raw/master/mortality/data_processing_elements_mortality-",
    checks$harmo_group,".xlsx"), 
  destfile = paste0("input_documents/data_processing_elements_mortality-",
                    checks$harmo_group, 
                    "-github.xlsx"),
  mode = "wb")
dpe_github <- read_excel_allsheets(paste0("input_documents/data_processing_elements_mortality-",
                                          checks$harmo_group, 
                                          "-github.xlsx"))
...
if(nrow(dpe_list) == 1){
  ...
  if(identical(dpe_local, dpe_github)){
    file.remove(paste0("input_documents/data_processing_elements_mortality-",
                       checks$harmo_group, 
                       "-github.xlsx"))
  }else{
    file.rename(
    ...
    file.rename(
      from = paste0("input_documents/data_processing_elements_mortality-",
                    checks$harmo_group, 
                    "-github.xlsx"),
      to = paste0("input_documents/data_processing_elements_mortality-",
                  checks$harmo_group, 
    ...
    if(checks$dataschema_uptodate != "Dataschema updated!"){
      ...
      if(specific == 2){
        ...
        write_excel_allsheets(dpe_github,
                              paste0("input_documents/data_processing_elements_mortality-",
                                     checks$harmo_group, 
                                     "-", format(checks$time_stamp, "%Y-%m-%d"), ".xlsx")
        )
        rm(var_changed)
...
if(checks$harmo_type == "all"){
  # --------------------------------------------
  # generate Maelstrom data dictionary and summary
  
  data_dictionary <- data_dict_extract(data_clean)
  ...
  write_excel_allsheets(
    data_dictionary,
    paste0("output_documents/input data dictionaries/","extracted_data_dict_mortality_",input_dataset$name, ".xlsx"))
  write_excel_allsheets(
    data_summary,
    paste0("output_documents/input data summaries/","data_summary_mortality_",input_dataset$name, ".xlsx"))
...
rm(list = c("data_dictionary", "data_summary", "DPE_vars_in_data",
