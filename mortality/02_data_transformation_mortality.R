...
if(any(checks$harmonization_errors,checks$harmonization_warnings)){
  ...
  message(
    "
Harmonization contains errors/warnings that may be associated with errors in the 
process.
...
if(!any(checks$harmonization_errors)){
  ...
  before_after_report_path <- 
    paste0("output_documents/","before_after_report_mortality-",
  ...
  output_summary_path <- 
    paste0("output_documents/","harmonization_summary_mortality-",
           checks$harmo_group,'.xlsx')
  write_excel_allsheets(
    output_summary,
    output_summary_path)
  ...
  harmonized_data_path <- 
    paste0("output_dataset/","harmonized_data_mortality-",
           checks$harmo_group,'.xlsx')
  write_excel_allsheets(
    pooled_harmonized_dataset,
    harmonized_data_path)
  ...
  harmonized_data_dict <- 
    data_dict_extract(pooled_harmonized_dataset)   # Change this to unpooled to save scripts?
  harmonized_data_dict_path <- 
    paste0("output_documents/","harmonized_data_dict_mortality-",
           checks$harmo_group,'.xlsx')
  write_excel_allsheets(
    harmonized_data_dict,
    harmonized_data_dict_path)
  ...
  # visual report
#  viz_path <- 
#    paste0("output_documents/","visual_report_mortality-",checks$harmo_group)
